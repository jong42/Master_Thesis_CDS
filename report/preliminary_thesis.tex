...
In the course of the thesis a simplified Bayesian model was created and visualized. Even this most simplified model did not behave as intuitively expected. Since it is very difficult to correctly assess the behavior of a Bayesian model by just looking at the code, especially for complex models, but even, as shown here, for the most simple ones, validating a model by visualizing it becomes very important. The interface implemented here makes it possible to check PyMC3 models in Lumen which can help spotting discrepancies between the model and the expectation of the researcher.\\
In terms of aggregating data Lumen currently is only able to compute points of highest probability density for PyMC3 models. Possible future work could therefore be directed towards the calculation of additional aggregations since graphically comparing aggregations between data and model is a widespread validation method.
Further possible improvements of Lumen include the integration of independent variables as well as further analyses on the performance on more complex models than the ones used in the thesis at hand.
TODO: Schlusssatz
