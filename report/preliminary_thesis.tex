...
Since "all models are wrong", it is necessary, after a Bayesian model has been found, to capture the extent to which the model fails to describe reality.
...
Possibility: Draw random samples from the predictive distribution, compare e.g. the smallest of these values with the smallest value in the data...

model checking by calculating \textit{test quantities}. \textit{test quantities} are scalar summaries of parameters that are used to compare data to simulations. \textit{test quantities} are similar to test statistics in the classical approach.
...
Stan is a program written in C++ that is designed for Bayesian inference on user-specified models. It consists of the following components:
\begin{itemize}
...
\end{itemize}
The code for describing a model by using the aforementioned modeling language is divided into four parts: \textit{data}, \textit{parameters}, \textit{transformed parameters} and \textit{model}: In \textit{data}, the structure of the input data, along with any constraints, is given (for example the data could be constrained to include only positive values). In \textit{parameters} and \textit{transformed parameters}, the parameters of the models are listed, whereat the former contains parameters which do not depend on other parameters, and the latter contains parameters which do. Finally, in \textit{model}, the model structure in the form of prior and likelihood distributions is given. Those four parts are shown in  \ref{fig:stan_example_code}, which is an example from \cite{Gelman_2015}, where the model $y=a_1e^{-b_1x} + a_2e^{-b_2x}$ is fit to data, using the PyStan interface of Stan for Python.
...
has plotting functions for posterior distributions
