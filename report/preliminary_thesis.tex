...
\end{split}
\end{equation}
Calculating this integral can be very difficult or impossible, since the integral can be multidimensional. So, it is not always possible to get the posterior distribution of the parameters analytically. However, there are methods to sample exactly from the posterior distribution of the parameters. 

\paragraph{Simple Monte Carlo}
The easiest of them is called Simple Monte Carlo. 
There we approximate the integral by sampling over the likelihood and the prior and then taking the mean over the samples:
\begin{equation}
...
\begin{enumerate}
	\item Draw a starting point $\theta^0$ from a starting distribution $p_0(\theta)$
	\item proposal distribution, also called jumping distribution

Metropolis requires the ability to compute a (not necessarily normalized) probability density for a given point.

generalization of the Gibbs sampler
\cite{1439840954}

In short: Do a random walk. If the probability density of the proposed point is higher than the probability density of the current point, go to the proposed point. Otherwise, only go to the proposed point with a certain probability and stay at the current point otherwise.
The distribution of the points (at least after convergence) have the property that they resemble the actual probability distribution one wants to find.

\end{enumerate}
\paragraph{Metropolis-Hastings}

Difference between Metropolis and Metropolis-Hastings: Calculation of the treshold at which the proposed point is accepted. Metropolis hastings additionally has the ratio between the proposal distributions in it. Metropolis is chosen in a way that these cancel out.
\paragraph{Hamiltonian Monte-Carlo}
less random walkbehaviour thanother MCMC algorithms
needs model parameters to be continuous
can reduce complexity from $O(D^2)$ (Metropolis) to $O(D^(5/4)$.
The algorithmhas parameters: a stepsize and a desired number of steps

auxiliary momentum variable $r_d$ for each modelvariable $\theta_d$.
The momentum variables aredrawn independently from the standard normal distribution.
...
Behaviour of $\theta$ and $r$ over time can be simulated through integration.

We have a momentum that leads to a new position of the parameters by applying the leapfrog integrator. This new position is then either accepted or rejected according to the metropolis algorithm.

$L$ is the number of integration steps to get to the next sample. $\epsilon$ is the stepsize  of one integration step.
...
\ref{hoffman2014no}
\paragraph{NUTS}
a self-tuning variant of Hamiltonian Monte Carlo \ref{Salvatier2016}.Uses gradient information about log-posterior density.
automatic differentiation of the log posterior density
less random walkbehaviour thanother MCMC algorithms
needs model parameters to be continuous
can reduce complexity from $O(D^2)$ (Metropolis) to $O(D^(5/4)$.
The algorithmhas parameters: a stepsize and a desired number of steps
...
PYMC3 uses a Metropolis within-Gibbs-sampler (https://docs.pymc.io/developer_guide.html)
...
\\
PyMC3 uses Theanoto improve performance
