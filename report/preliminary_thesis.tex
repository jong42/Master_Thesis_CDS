...
\paragraph{NUTS}
The No-U-Turn-Sampler (NUTS) is a self-tuning variant of Hamiltonian Monte Carlo \cite{Salvatier2016}. The number of steps per sample $L$ does not need to be specified at a fixed value anymore. Instead, $L$ is automatically found by going forward and backward from the initial position, until the change of location does not increase the distance between initial and proposed location anymore \cite{hoffman2014no}.

%goal is to maximize distance between previous and proposed sample. Avoid behaviour where additional steps lead back to previous sample.


%product of the distance (between initial and proposed sample) and the momentum $r$, that is a measure of how much the distance increases with additional steps. If this measure becomes zero, there is no need to do further steps and the optimal number of steps is reached.


%reversibility has to be preserved. That's why the algorithm searches forward and backward.
