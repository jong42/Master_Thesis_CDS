...
As a basic example to implement in PyMC3 we choose the model from \autoref{sec:Comparison}. The expectation here is that the resulting Lumen plot should look much like the visualizations in \autoref{fig:code_use_case_1}. The model contains the random variables $X$ and $mu$, the data for it was generated by the following  distributions:
\begin{equation}
\begin{split}
X &\sim N(\mu,1), \\
\mu &\sim N(0,1),
...
\begin{figure}[h]
	...
	\end{lstlisting}
	\caption[PyMC3 model of use case 2]{PyMC3 model of a linear regression model}
...
\begin{figure}[h]
	...
	\end{lstlisting}
	\caption[PyMC3 model of the eight schools example]{PyMC3 model of the eight schools example}
	\label{fig:eight_schools_PyMC3_code}
\end{figure}
\begin{figure}
...
%Sinharay et al then fit a different model with an exponential rather than a uniform prior, do the tests again, and find that only some of them give worse results in the second model, as they should, since the model is false. 
 ...
 \section{Discussion}
Korrektheit des Interfaces, shortcomings?
Kann es für generische Modelle angewendet werden?
Was wurde noch nicht probiert
\\
Nutzen der Visualisierung für user von PyMC3 modellen
Wer sind user von PyMC3 modellen, welche MOdelle benutzen sie, wie kann visualisierung dafür hilfreich sein?
\\
...
\\
Discuss conclusions of the examples
\\
