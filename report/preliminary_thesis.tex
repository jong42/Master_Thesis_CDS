...
%According to \cite{Wang2018}, the likelihood of a new data point can be calculated by integrating the product of the prior likelihood and conditional probability of $\beta$ over the space of $\beta$, as shown in equation \ref{eq:posterior_predictive}. This formula can also be derived using the chain rule (I think).
%\\
...
Traditionally, model checking has been performed by computing p-values, which give the probability that an observed outcome occurs under the model in question. Gelman \cite{gelman2004exploratory} argues that visualization can be seen as an alternative form of model checking. He even claims that "when more complex models are being used, graphical checks are more necessary than ever to detect areas of model misfit" \cite{gelman2004exploratory}.\\
The idea of any model check is to compare observed data to simulated data generated under the model assumptions. This is done by choosing a summarizing measure of the data, called a test statistic. The test statistics for the observed and the simulated data can then be compared. The p-value is then just the representation of the discrepancy between the two test statistics. Test statistics are usually scalars, but in graphical model checking the visualization plays the role of the test statistic \cite{gelman2004exploratory}.

%Can also the data itself be seen as a test statistic?
