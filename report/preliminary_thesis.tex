...
\usepackage{hyperref} % reference chapters. Has to be loaded before babel is loaded for correct capitalization
...
% In Lumen: One can check the models behavior, that is, the interaction of pairs of variables.
...
As one can see in \autoref{fig:ground_truth_posterior_6}, the distribution looks very similar. The histogram for the variable $mu$ is not shown there, since $mu$ is no longer part of the data generating mechanism. In the model, $mu$ is still considered and a posterior distribution for it is learned. The similarity between \autoref{fig:ground_truth_posterior_1} and \autoref{fig:ground_truth_posterior_6} is not surprising since the data generated from the according mechanisms in \autoref{eq:data_generating_manual_example} and \autoref{eq:nonhierarchical} looks very similar, even if the actual data generating mechanisms are different. 
% The property that different mechanisms can generate the same data and are therefore indistinguishable, is called Nonidentifiability.
...
\textit{This section describes how the main goal of the thesis is achieved: The implementation of an interface that enables Lumen to display models built in PyMC3.}\\
\linebreak
...
\textit{In this section, example models are created in PyMC3 to show how the finished interface performs in practice.}
...
%As seen in \autoref{sec:Comparison}, Bayesian models do not always behave as one would first expect. It might not have practical implications but contributes to a better understanding of the behavior of Bayesian models.
...
%Further potential improvements of Lumen include the integration of independent variables as well as 
%
%\\
%Lumen is already providing useful features for performing visual analytics. By realizing these improvements it %could develop its full potential as a powerful tool for validating and exploring probabilistic models.
